	...
	<?php 
		...
		function LayDanhSachSanPhamMoi(){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MASP"],'GIATIEN'=> $dongtienich["GIA"],'TENSP' => $dongtienich["TENSP"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME']."/weblazada".$dongtienich["ANHLON"]));
		...
		function LayLogoThuongHieuLon(){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MATHUONGHIEU"],'TENSP' => $dongtienich["TENTHUONGHIEU"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME']."/weblazada/".$dongtienich["HINHTHUONGHIEU"]));
				...
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					if($ketquacon){
						while ($dongphukiencon = mysqli_fetch_array($ketquacon)) {
							array_push($chuoijson, array("MASP"=>$dongphukiencon["MALOAISP"],'TENSP' => $dongphukiencon["TENLOAISP"],'GIATIEN'=>$dongphukiencon["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME']."/weblazada".$dongphukiencon["ANHLON"]));
		...
		function LayDanhSachCacThuongHieuLon(){
			...
			if($ketqua){
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					//cách 2
					array_push($chuoijson, array("MASP"=>$dong["MATHUONGHIEU"],'TENSP' => $dong["TENTHUONGHIEU"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME']."/weblazada".$dong["HINHLOAISPTH"]));
					//end cách 2
				}
